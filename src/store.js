    ...
    sendIndication: async (ctx, data) => {
      ...
      if (res.ok) {
        ...
        for(let i = 0; i < json.length; i++) {
          if( json[i].error ){
            str = [...str, {id: i,error: json[i].error, message: json[i].message}]
          } else {
            for(let j = 0; j < json[i].data.length; j++) {
              str = [...str, {id: j, error: json[i].data[j].error, message: json[i].data[j].message}]
            }
          }
        }
        ctx.commit("notice", str)
